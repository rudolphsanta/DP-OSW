...
legend("topleft", legend = c("Static airfoil blades, extreme turbulence model", 
       ...
       col = c("red","red","black","black"), lty = c(1,3, 1, 3), lwd = 3.5, cex = 1.5)
...
low.switchover <- (6600000 + 4100000 - 3*Anchor.cost(MBL))/(3*R4.weight*R4.cost.high*chain.to.depth.ratio)
high.switchover <- (11500000 + 3100000 - 3*Anchor.cost(MBL))/(3*R4S.weight*R4.cost.low*chain.to.depth.ratio)
# best.switchover <- 
#####################################################################################
